...
from .CaptionModel_preh import CaptionModel
...
class AttModel(CaptionModel):
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None):
        ...
        for i in range(seq.size(1) - 1):
            ...
            output, state, pre_h = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state, pre_h)
            outputs[:, i] = output

        return outputs         # batch * seq_len * vocab_size

    def get_logprobs_state(self, it, fc_feats, att_feats, p_att_feats, att_masks, state, pre_h):
        # 'it' contains a word index   it, tmp_fc_feats, tmp_att_feats, tmp_p_att_feats, tmp_att_masks, state, pre_h
        xt = self.embed(it)

        output, state, pre_h = self.core(xt, fc_feats, att_feats, p_att_feats, state, pre_h, att_masks,)
        logprobs = F.log_softmax(self.logit(output), dim=1)
        return logprobs, state, pre_h
    ...
    def _sample_beam(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        for k in range(batch_size):
            ...
            for t in range(1):
                ...
                logprobs, state, pre_h = self.get_logprobs_state(it, tmp_fc_feats, tmp_att_feats, tmp_p_att_feats, tmp_att_masks, state, pre_h)

            self.done_beams[k] = self.beam_search(pre_h, state, logprobs, tmp_fc_feats, tmp_att_feats, tmp_p_att_feats, tmp_att_masks, opt=opt)
    ...
    def _sample(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        for t in range(self.seq_length + 1):
            ...
            logprobs, state, pre_h = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state, pre_h)
...
class TopDownCore(nn.Module):
    def __init__(self, opt, use_maxout=False):
        ...
        self.att_v2h = Attention_v2h(opt)
        self.attention = Attention(opt)

        self.fusion = opt.fusion_way
        self.sig = nn.Sigmoid()

        if self.fusion == 1:
            self.W_g = nn.Linear(opt.rnn_size * 3, opt.rnn_size)
        elif self.fusion == 2 or self.fusion == 3:
            self.W_att = nn.Linear(opt.rnn_size, opt.rnn_size)
            self.W_attv2h = nn.Linear(opt.rnn_size, opt.rnn_size)

    def forward(self, xt, fc_feats, att_feats, p_att_feats, state, pre_h, att_masks=None):
        ...
        if self.fusion == 3:
            # dont use histroy semantic information, equal that current hidden state (h't) has all attention score(1)
            att_v2h = h_att
            g = self.sig(self.W_att(att) + self.W_attv2h(att_v2h))
            lang_lstm_input = torch.cat([g.mul(att), (1 - g).mul(att_v2h), h_att], 1)
        
        else:
            h_curr = h_att.unsqueeze(1)
            pre_h = torch.cat([pre_h, h_curr], 1)
            att_v2h = self.att_v2h(fc_feats, pre_h)

            if self.fusion == 1:
                # gate fusion 1
                # g = sigmoid(W_g*[att, att_v2h, h_att])
                # lang_input = [g*att, (1-g)*att_v2h, h_att]
                fuse_tensor = torch.cat([att, att_v2h, h_att], 1)
                g = self.sig(self.W_g(fuse_tensor))
                lang_lstm_input = torch.cat([g.mul(att), (1 - g).mul(att_v2h), h_att], 1)


            elif self.fusion == 2:
                # gate fusion 2
                # g = sigmoid(W_att * att + W_attv2h * att_v2h)
                # lang_input = [g * att, (1-g) * att_v2h, h_att]
                g = self.sig(self.W_att(att) + self.W_attv2h(att_v2h))
                lang_lstm_input = torch.cat([g.mul(att), (1 - g).mul(att_v2h), h_att], 1)

        #lang_lstm_input = torch.cat([att, att_v2h, h_att], 1)
...
# class Attention_v2h(nn.Module):
#     def __int__(self, opt):
#         super(Attention_v2h, self).__init__()
#         self.rnn_size = opt.rnn_size
#         self.att_hid_size = opt.att_hid_size
#         self.W_v = nn.Linear(self.att_hid_size, self.rnn_size)
#         self.W_v = nn.Linear(self.att_hid_size, self.rnn_size)
#         self.alpha_net = nn.Linear(self.att_hid_size, 1)
#
#     def forward(self, fc_feats, h_1):
#         p_fc_feats = self.W_v(fc_feats)
#         p_h = self.W_h(h_1)
#
#         v_h = p_fc_feats.unsqueeze(1).expand_as(p_h)
#         dot = p_h + v_h
#         dot = F.tanh(dot)
#         dot = dot.view(-1, self.att_hid_size)
#         dot = self.alpha_net(dot)
#         dot = dot.view(-1, h_1.size(1))
#
#         weight = F.softmax(dot)
#         att_res = torch.bmm(weight.unsqueeze(1), h_1).squeeze(1)
#
#         return att_res
...
class Attention_v2h(nn.Module):
    def __init__(self, opt):
        ...
        #self.linear = nn.Linear(self.rnn_size, self.att_hid_size)
        self.W_h = nn.Linear(self.att_hid_size, self.rnn_size)
        self.W_v = nn.Linear(self.att_hid_size, self.rnn_size)
        self.alpha_net = nn.Linear(self.att_hid_size, 1)

    def forward(self, fc_feats, h_1):
        # p_fc_feats 50 * 512   h_1: 50 * (t-1) * 512
        # Project the attention feats first to reduce memory and computation comsumptions.
        p_fc_feats = self.W_v(fc_feats)       # W*V
        p_h = self.W_h(h_1)  #  50 * (t-1) *512      W*h

        v_h = p_fc_feats.unsqueeze(1).expand_as(p_h)    #50 * 512 *512
        dot = p_h + v_h
        dot = F.tanh(dot)
        dot = dot.view(-1, self.att_hid_size)  # (batch * att_size) * att_hid_size
        dot = self.alpha_net(dot)  # (batch * att_size) * 1
        dot = dot.view(-1, h_1.size(1))  # batch * att_size

        weight = F.softmax(dot)  # batch * att_size
        att_res = torch.bmm(weight.unsqueeze(1), h_1).squeeze(1)  # batch * att_feat_size         weight.unsqueeze(1): batch,1,512
